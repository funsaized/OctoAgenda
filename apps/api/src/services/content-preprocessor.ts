...
const EVENT_SELECTORS = [
  '.event', '.events', '.event-item', '.event-container',
  '.calendar-event', '.upcoming-event', '.event-listing',
  '[class*="event"]', '[id*="event"]',
  'article', 'section.events', 'div.calendar',
...
export function hasEventContent(processedContent: ProcessedContent): boolean {
  return processedContent.eventContainers.length > 0 ||
